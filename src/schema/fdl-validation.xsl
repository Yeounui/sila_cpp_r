...
<!-- OVERLAY of third_party/sila_base/xslt/fdl-validation.xsl (submodule pin
     878903c0d1026b96b325e947e9c59baf8b265f51, sila_base v1.2). Two changes vs
     upstream, both because Part A p66 / Part B p83 explicitly allow a SiLA
     Constrained Type to be based on another SiLA Constrained Type (the two
     constraint layers act as a logical AND):
       1. detect-invalid-constraint-base-type: the xsl:when on
          sila:DataType/sila:Constrained (a blanket rejection of that nesting)
          is removed.
       2. detect-invalid-constraint: each constraint-applicability arm tests
          the ULTIMATE base type (the first sila:Basic or sila:List reached
          through nested Constrained/DataType chains, bound to $base) instead
          of the immediate sila:DataType child, so an outer constraint is
          still checked against the type it finally applies to (an outer
          Pattern over Constrained(Integer) stays invalid).
     sila_base is a submodule that must not be edited in place; regenerate
     this file on a submodule bump; tests/codegen/test_fdl_validation.py's
     XSLT drift guard enforces that ONLY this header comment, that one
     three-line xsl:when, the $base variable line and the arm tests differ
     from the submodule copy. -->
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="detect-invalid-constraint">
        <xsl:for-each select="//sila:Constrained">
            <xsl:variable name="base" select="(sila:DataType//*[self::sila:Basic or self::sila:List])[1]"/>
            <xsl:choose>
                <xsl:when test="$base/self::sila:Basic/text() = 'String'">
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Integer'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Set'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Real'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Set'
                                or local-name() = 'MaximalInclusive'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Binary'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Length'
                                or local-name() = 'MinimalLength'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Date'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Set'
                                or local-name() = 'MaximalInclusive'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Time'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Set'
                                or local-name() = 'MaximalInclusive'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Timestamp'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                                local-name() = 'Set'
                                or local-name() = 'MaximalInclusive'
                ...
                <xsl:when test="$base/self::sila:Basic/text() = 'Any'">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
                ...
                <xsl:when test="$base/self::sila:List">
                    <xsl:for-each select="sila:Constraints/*">
                        <xsl:if test="not(
