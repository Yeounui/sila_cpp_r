...
// where this port deliberately departs from the original (OpenSSL 3.0 keygen,
...
{
    // EVP_RSA_gen casts bits to size_t, where a negative value wraps to a huge
    // request instead of failing; generateKey rejects it before the macro.
    EXPECT_THROW(sila2::generateKey(0), sila2::OpenSslError);
    EXPECT_THROW(sila2::generateKey(-1), sila2::OpenSslError);
}
...
TEST(TlsConfig, EmbedsTheServerUuidOnlyForTheSila2Hostname)
{
    ...
    EXPECT_LT(X509_get_ext_by_OBJ(otherCert.get(), uuidOid.get(), -1), 0);
}
